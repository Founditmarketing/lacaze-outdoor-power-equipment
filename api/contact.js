...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "service\@lacazeoutdoor\.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Lacaze Outdoor Power Equipment <leads@lacazeoutdoor.com>";

  const subject = `New lead from lacazeoutdoor.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on lacazeoutdoor.com`;
  const html = `
<h2>New lead from Lacaze Outdoor Power Equipment</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on lacazeoutdoor.com</p>`;
